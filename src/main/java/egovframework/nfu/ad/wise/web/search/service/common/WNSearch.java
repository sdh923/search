...
public class WNSearch {
	...
	public WNSearch(String searcherId, boolean isDebug, boolean isUidSrch, 
	    ...
	    if(searchFields!=null){
	    	this.searchFields = new String[searchFields.length];
		    for(int i = 0; i < searchFields.length; ++i)
		    	this.searchFields[i] = searchFields[i];
	    }
	...
	public WNSearch(String searcherId, boolean isDebug, boolean isUidSrch, 
	    ...
	    if(mergeCollections!=null){
	    	this.mergeCollections = new String[mergeCollections.length];
		    for(int i = 0; i < mergeCollections.length; ++i)
		    	this.mergeCollections[i] = mergeCollections[i];
	    }
	    if(searchFields!=null){
	    	this.searchFields = new String[searchFields.length];
		    for(int i = 0; i < searchFields.length; ++i)
	...
	public int setBoostQuery(int idx) {
		...
		if (!wncol.collectionInfo[idx][WNCollection.BOOST_QUERY].equals("")) {
			/* if(isDebug) {
				checkCollectionQueryFieldName("[w3SetBoostQuery] [setBoostQuery]", collections[idx],wncol.collectionInfo[idx][COLLECTION_QUERY] );
			} */
			ret = common.setBoostQuery(wncol.collections[idx], wncol.collectionInfo[idx][WNCollection.BOOST_QUERY]);
		}
		return ret;
	}
	...
	*/
	...
	public int getCollIdx(String collName) {
		for(int i=0; i < wncol.collections.length; i++) {
			if (wncol.collections[i].trim().equals(collName.trim())) return i;
		}
		return -1;
	}
	...
	public int getMergeCollIdx(String collName) {
		for(int i=0; i < wncol.mergeCollections.length; i++) {
			if( wncol.mergeCollections[i].trim().equals(collName.trim()) ) return i;
		}
		return -1;
	}
	...
	*/
	...
	public void debugMsg(boolean isRealQuery, String query){
		...
		sb.append("<br>[w3ConnectServer] [IP] ").append(sf1Vo.getSearchIp()).append(" [PORT] ").append(sf1Vo.getSearchPort()).append(" [TIMEOUT]").append(sf1Vo.getSearchTimeOut());
		if(!isUidSrch) {
			sb.append("<br>[w3RecvResult] Mode : ").append(this.connectionOpt);
			if(isRealQuery){
				sb.append("<br>[W3ReceiveRecentQueryListResult] Set Count : ").append(WNCollection.REALTIME_COUNT);
			}
		}
		...
		for(int i=0; i < collections.length; i++) {
			...
			if(content != null && !content.equals("")){
				sb.append("<br>[w3AddCollection] <b>").append(content).append("</b>");
			}
			if(!isUidSrch) {
				//페이지 설정
				content = wncol.collectionInfo[idx][WNCollection.PAGE_INFO];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetPageInfo] ").append(content);
				}

				//분석 정보
				content = wncol.collectionInfo[idx][WNCollection.ANALYZER];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetQueryAnalyzer] ").append(content);
				}
				...
				if(count >= 0){
					...
					sb.append("<br>[w3SetQueryAnalyzer] ").append(count).append(content);
				}

				//정렬 정보
				content = wncol.collectionInfo[idx][WNCollection.SORT_FIELD];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3AddSortField] ").append(content);
				}
				...
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetDateRange] ").append(content);
				}

				//확장 검색  정보
				content = wncol.collectionInfo[idx][WNCollection.EXQUERY_FIELD];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetPrefixQuery] ").append(content.replaceAll("<", "&lt;").replaceAll(">", "&gt;"));
				}

				//카테고리 부스트 정보
				content = wncol.collectionInfo[idx][WNCollection.CATEGORY_BOOST];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetBoostCategory] ").append(content);
				}
				...
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetCollectionQuery] ").append(content.replaceAll("<", "&lt;").replaceAll(">", "&gt;"));
				}

				//필터 정보
				content = wncol.collectionInfo[idx][WNCollection.FILTER_OPERATION];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetFilterQuery] ").append(content.replaceAll("<", "&lt;").replaceAll(">", "&gt;"));
				}

				//그룹화  정보
				content = wncol.collectionInfo[idx][WNCollection.GROUP_BY];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetGroupBy] ").append(content);
				}

				//그룹 정렬  정보
				content = wncol.collectionInfo[idx][WNCollection.GROUP_SORT_FIELD];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3AddSortFieldInGroup] ").append(content);
				}
				...
				if(!wncol.collectionInfo[idx][WNCollection.GROUP_BY].equals("")){
					...
					if(count > 0){
						sb.append("<br>[w3GetGroupBycount] ").append(count);
					}
				}
			}
			...
			if(content != null && !content.equals("")){
				sb.append("<br>[w3AddDocumentField] ").append(content);
			}
			sb.append("<br>");
		}
		...
		if(mergeCollections != null) {
			for(int i=0; i < mergeCollections.length; i++) {
				...
				if(content != null && !content.equals("") && mappingColl != null && !mappingColl.equals("")){
					sb.append("<br>[w3AddMergeCollection] <b>").append(content).append("</b>, [Collection Name]<b>").append(mappingColl).append("</b>");
				}
				
				content = wncol.mergeCollectionInfo[idx][WNCollection.MERGE_PAGE_INFO];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetMergePageInfo] ").append(content);
				}
				...
				if(content != null && !content.equals("") && mappingDocs != null && !mappingDocs.equals("")){
					sb.append("<br>[w3AddMergeDocumentField] ").append(content).append(", [Collection Document Fields]").append(mappingDocs);
				}
				...
				if(content != null && !content.equals("") && mappingMulti != null && !mappingMulti.equals("")){
					sb.append("<br>[w3AddMergeMultiGroupByField] ").append(content).append(", [Collection Multi-Groupby Fields]").append(mappingMulti);
				}
				...
				if(content != null && !content.equals("") && mappingCategory != null && !mappingCategory.equals("")){
					sb.append("<br>[w3AddMergeCategoryGroupByField] ").append(content).append(", [Collection Category Groupby Fields]").append(mappingCategory);
				}
			}
