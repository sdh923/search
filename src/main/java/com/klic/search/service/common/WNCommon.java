...
public class WNCommon {
	...
	*/
	 ...
	 * @param isDubug
	 */
	public WNCommon(boolean isDubug) {
		this.search = new Search();
	...
	public int setCommonQuery(String query, String charSet) {
		...
		loggerInfo(ret,"[w3SetCodePage()]["+charSet+"]");
		ret = search.w3SetQueryLog(1);
		loggerInfo(ret,"[w3SetQueryLog()][1]");
		ret = search.w3SetTraceLog(0);
		loggerInfo(ret,"[w3SetTraceLog()][0]");
		ret = search.w3SetCommonQuery(query, 0);
		loggerInfo(ret,"[w3SetCommonQuery()]["+query+"][0]");
	...
	public int setUidQuery(String charSet) {
		...
		loggerInfo(ret,"[w3SetQueryLog()][0]");
		return ret;
	}
	...
	public int setCommonQuery(String query, String charSet, int useOrResult, boolean useSuggestedQuery) {
		...
		loggerInfo(ret,"[w3SetCommonQuery()]["+query+"]["+useOrResult+"]");
		if(useSuggestedQuery) {
			ret = search.w3SetSpellCorrectionQuery(query, 0);
			loggerInfo(ret,"[w3SetSpellCorrectionQuery()]["+query+"][0]");
		}
		return ret;
	}
	...
	public int setCommonQuery(String query, String charSet, String[] logInfo) {
		...
		if (logInfo != null && logInfo.length > 2) {
			...
			loggerInfo(ret,"[w3SetSessionInfo()]["+query+"]["+logInfo[0]+"]["+logInfo[1]+"]["+logInfo[2]+"]");
		}
	...
	public int setCollectionQuery(String collectionName, String query) {
		int ret = 0;
		ret = search.w3SetCollectionQuery(collectionName, query);
		loggerInfo(ret,"[w3SetCollectionQuery()]["+collectionName+"]["+query+"]");
		return ret;
	}
	...
	public int setBoostQuery(String collectionName, String query) {
		...
		ret = search.w3SetBoostQuery(collectionName, query, option);
		loggerInfo(ret,"[w3SetBoostQuery()]["+collectionName+"]["+query+"]["+option+"]");
		return ret;
	}
	...
	public int setUid(String collectionName, String[] values, String searcherId) {
		...
		for (int i = 0; i < values.length; i++) {
			...
			loggerInfo(ret,"[w3AddUid()]["+collectionName+"]["+uid+"]["+searcherId+"]");
		}
		return ret;
	}
	...
	public int addCollection(String collectionName, int useKma, int isCase) {
		...
		loggerInfo(ret,"[w3AddCollection()]["+collectionName+"]");
		//USEMA, ISCASE, USEORIGINAL, USESYNONYM
		ret = search.w3SetQueryAnalyzer(collectionName, useKma, isCase, 1,1);
		loggerInfo(ret,"[w3SetQueryAnalyzer()]["+collectionName+"]["+useKma+"]["+isCase+"][1][1]");
		ret = search.w3SetDuplicateDetection(collectionName);
		loggerInfo(ret,"[w3SetDuplicateDetection()]["+collectionName+"]");
	...
	public int addCollection(String collectionName, int useKma, int isCase, 
		...
		loggerInfo(ret,"[w3SetQueryAnalyzer()]["+collectionName+"]["+useKma+"]["+isCase+"]["+useOriginal+"]["+useSynonym+"]");
		//systemOut("[w3AddCollection] "+collectionName);
	...
	public int addAliasCollection(String aliasName, String collectionName,
	  int useKma, int isCase) {
		...
		loggerInfo(ret,"[w3AddAliasCollection()]["+aliasName+"]["+collectionName+"]");
		ret = search.w3SetQueryAnalyzer(aliasName, useKma, isCase, 1, 1);
		loggerInfo(ret,"[w3SetQueryAnalyzer()]["+aliasName+"]["+useKma+"]["+isCase+"][1][1]");
		ret = search.w3SetDuplicateDetection(aliasName);
		loggerInfo(ret,"[w3SetDuplicateDetection()]["+aliasName+"]");
		return ret;
	}
	...
	public int addAliasCollection(String aliasName, String collectionName,
	  int useKma, int isCase, int useOriginal, int useSynonym, int duplicateDectection) {
		...
		ret = search.w3AddAliasCollection(aliasName, collectionName);
		loggerInfo(ret,"[w3AddAliasCollection()]["+aliasName+"]["+collectionName+"]");
		ret = search.w3SetQueryAnalyzer(aliasName, useKma, isCase, useOriginal, useSynonym);
		loggerInfo(ret,"[w3SetQueryAnalyzer()]["+aliasName+"]["+useKma+"]["+isCase+"]["+useOriginal+"]["+useSynonym+"]");
		if(duplicateDectection == 1) {
			ret = search.w3SetDuplicateDetection(aliasName);
			loggerInfo(ret,"[w3SetDuplicateDetection()]["+aliasName+"]");
		}
	...
	public int getConnection(String ip, int port, int timeOut) {
		...
		loggerInfo(ret,"[w3ConnectServer()]["+ip+"]["+port+"]["+timeOut+"]");
		return ret;
	}
	...
	public int addSearchField(String collectionName, String[] fields) {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			loggerInfo(ret,"[w3AddSearchField()]["+collectionName+"]["+fields[i]+"]");
	...
	public int addSearchFieldScore(String collectionName, String[] fields, int[] rankScores) {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			loggerInfo(ret,"[w3AddSearchFieldScore()]["+collectionName+"]["+fields[i]+"]["+rankScores[i]+"]");
	...
	public int setSearchField(String collectionName, String fields) {
		...
		for (int i = 0; i < searchFieldTemp.length; i++) {
			...
			if (searchField != null && searchField.length > 0) {
				...
				loggerInfo(ret,"[w3AddSearchField()]["+collectionName+"]["+searchField[0]+"]");
				//systemOut("[w3AddSearchField] " + collectionName + " / "+ searchField[0]);
			}
		}
		ret = search.w3SetSearchFieldScore(collectionName, fields);
		loggerInfo(ret,"[w3SetSearchFieldScore()]["+collectionName+"]["+fields+"]");
	...
	public int addSortField(String collectionName, String sortField, int sortOrder) {
		...
		loggerInfo(ret,"[w3AddSortField()]["+collectionName+"]["+sortField+"]["+sortOrder+"]");
	...
	public int setSortField(String collectionName, String sortValue) {
		...
		loggerInfo(ret,"[w3SetSortField()]["+collectionName+"]["+sortValue+"]");
	...
	*/
	public int setRanking(String collectionName, String method, String option, int maxcount) {
		int ret = 0;
		ret = search.w3SetRanking(collectionName, method, option, maxcount);
		loggerInfo(ret,"[w3SetRanking()]["+collectionName+"]["+method+"]["+option+"]["+maxcount+"]");
		return ret;
	}
	...
	*/
	public int setRankRange(String collectionName, int minRank, int maxRank) {
		int ret = 0;
		ret = search.w3SetRankRange(collectionName, minRank, maxRank);
		loggerInfo(ret,"[w3SetRankRange()]["+collectionName+"]["+minRank+"]["+maxRank+"]");
		return ret;
	...
	public int setPrefixQuery(String collectionName,
		...
		loggerInfo(ret,"[w3SetPrefixQuery()]["+collectionName+"]["+prefixQuery+"]["+operator+"]");
	...
	public int setFilterOperation(String collectionName, String fieldNameValues) {
		...
		loggerInfo(ret,"[w3SetFilterQuery()]["+collectionName+"]["+fieldNameValues+"]["+fieldNameValues+"]");
	...
	public int setFilterQuery(String collectionName, String fieldNameValues) {
		int ret = search.w3SetFilterQuery(collectionName, fieldNameValues);
		loggerInfo(ret,"[w3SetFilterQuery()]["+collectionName+"]["+fieldNameValues+"]");
		//systemOut("[w3SetFilterQuery]" + fieldNameValues);
		return ret;
	}
	...
	*/
	public int setResultField(String collectionName, String[] fields) {
		...
		for(int i=0; i< fields.length; i++) {
			...
			loggerInfo(ret,"[w3AddDocumentField()]["+collectionName+"]["+fields[i]+"][0]");
	...
	public int setResultField(String collectionName, String fields) {
		...
		loggerInfo(ret,"[w3SetDocumentField()]["+collectionName+"]["+fields+"]");
	...
	public int setPageInfo(String collName, int highlight, int startPos,
		...
		if (highlight == 1) { //OFF, ON
			...
			loggerInfo(ret,"[w3SetHighlight()]["+collName+"][0][1]");
		} else if (highlight == 2) { //ON , OFF
			ret = search.w3SetHighlight(collName, 1, 0);
			loggerInfo(ret,"[w3SetHighlight()]["+collName+"][1][0]");
		} else if (highlight == 3) { //ON, ON
			ret = search.w3SetHighlight(collName, 1, 1);
			loggerInfo(ret,"[w3SetHighlight()]["+collName+"][1][1]");
		} else {//OFF, OFF
			ret = search.w3SetHighlight(collName, 0, 0);
			loggerInfo(ret,"[w3SetHighlight()]["+collName+"][0][0]");
		}
		// 페이지, 기본정렬 설정
		ret = search.w3SetPageInfo(collName, startPos, resultCnt);
		loggerInfo(ret,"[w3SetPageInfo()]["+collName+"]["+startPos+"]["+resultCnt+"]");
	...
	public int setDateRange(String collectionName, String startDate,
	  String endDate, String replaceChr) {
		...
		if (!startDate.equals("") && !endDate.equals("")) {
			...
			loggerInfo(ret,"[w3SetDateRange()]["+collectionName+"]["+startDate+"]["+endDate+"]");
		}
		return ret;
	}
    ...
    */
	public int setCategoryBoost(String collectionName, String field, 
		...
		loggerInfo(ret,"[w3SetBoostCategory()]["+collectionName+"]["+field+"]["+matchType+"]["+boostKeyword+"]");
		return ret;
	}
	...
	public int addDuplicateDetectionCriterionField(String collection, String field, int nOrder) {  	
		...
		loggerInfo(ret,"[w3AddDuplicateDetectionCriterionField()]["+collection+"]["+field+"]["+nOrder+"]");
		return ret;
	}
	...
	public int setDuplicateDetectionCriterionField(String collection, String fieldList ) {  	
		...
		loggerInfo(ret,"[w3SetDuplicateDetectionCriterionField()]["+collection+"]["+fieldList+"]");
		return ret;
	}
		...
		for(int i=0; i < length; i++) {
			String[] dataFields = WNUtils.split(categoryGroup[i], ":");
				if(dataFields.length == 2){
					...
					loggerInfo(ret,"[w3AddCategoryGroupBy()]["+collectionName+"]["+dataFields[0]+"]["+dataFields[1]+"]");
				}
		}
		return ret;
	}
	...
	public int addCategoryQuery(String collectionName, String[] categoryQuery) {
	  	...
	  	for(int i=0; i < length; i++) {
			...
			if(dataFields.length == 2){
				...
				loggerInfo(ret,"[w3AddCategoryQuery()]["+collectionName+"]["+dataFields[0]+"]["+dataFields[1]+"]");
			}
	  	}
	  	return ret;
	}
	...
	*/
	public int setPropertyGroup(String collectionName, String field, int min, int max, int groupCount) {
		...
		loggerInfo(ret,"[w3SetPropertyGroup()]["+collectionName+"]["+field+"]["+min+"]["+max+"]["+groupCount+"]");
		return ret;
	}
	...
	public int setGroupBy(String collectionName, String field, int docCount) {
		...
		loggerInfo(ret,"[w3SetGroupBy()]["+collectionName+"]["+field+"]["+docCount+"]");
		return ret;
	}
	...
	public int addMultiGroupBy(String collectionName, String fields) {
		...
		for(int i = 0; i < length; i++ ) {
			...
			loggerInfo(ret,"[w3AddMultiGroupBy()]["+collectionName+"]["+field[i]+"]");
		}
		return ret ;
	}
	...
	public int setMultiGroupBy (String collectionName, String fields) {
		int ret = 0;
		ret = search.w3SetMultiGroupBy(collectionName, fields);
		loggerInfo(ret,"[w3SetMultiGroupBy()]["+collectionName+"]["+fields+"]");
		return ret;
	}
	...
	public int addSortFieldInGroup(String collectionName, String sortField,
	  int sortOrder) {
		...
		loggerInfo(ret,"[w3AddSortFieldInGroup()]["+collectionName+"]["+sortField+"]["+sortOrder+"]");
	...
	public int setSortFieldInGroup(String collectionName, String sortValue) {
		...
		loggerInfo(ret,"[w3SetSortFieldInGroup()]["+collectionName+"]["+sortValue+"]");
	...
	public int setAuthorityQuery(String collection, String authTargetField,
	  String authCollection, String authReferField, String authorityQuery ) {
		...
		loggerInfo(ret,"[w3SetAuthorityQuery()]["+collection+"]["+authTargetField+"]["+authCollection+"]["+authReferField+"]["+authorityQuery+"]");
	...
	public String getHighlightKeywordByField(String colName,
	  String searchField) {
		String keyWord = this.search.w3GetHighlightByField(colName, searchField);
		return keyWord;
	}
	...
	public String[] receiveRecentQueryListResult(int mode, int count) {
		...
		loggerInfo(ret,"[w3ReceiveRecentQueryListResult()]["+mode+"]["+count+"]");
		if( ret < 0 ){
	...
	public int receiveRecentQueryListResultAsXml(int mode, int count) {
		...
		loggerInfo(ret,"[w3ReceiveRecentQueryListResultAsXml()]["+mode+"]["+count+"]");
		if( ret < 0 ){
	...
	public int receiveRecentQueryListResultAsJson(int mode, int count) {
		...
		loggerInfo(ret,"[w3ReceiveRecentQueryListResultAsJson()]["+mode+"]["+count+"]");
		if( ret < 0 ){
	...
	*/
	...
	public int recvResult(int mode) {
		...
		loggerInfo(ret,"[w3ReceiveSearchQueryResult()]["+mode+"]");
		return ret;
	}
	...
	*/
	public int recvDuplicateDocumentsResult(int mode) {
		...
		loggerInfo(ret,"[w3ReceiveDuplicateDocumentsResult()]["+mode+"]");
		return ret;
	}
	...
	*/
	public int recvDuplicateDocumentsResultAsJson(int mode) {
		...
		loggerInfo(ret,"[w3ReceiveDuplicateDocumentsResultAsJson()]["+mode+"]");
		return ret;
	}
	...
	*/
	public int recvDuplicateDocumentsResultAsXml(int mode) {
		...
		loggerInfo(ret,"[w3ReceiveDuplicateDocumentsResultAsXml()]["+mode+"]");
		return ret;
	}
	...
	public int receiveSearchQueryResultAsXml(int mode) {
		...
		loggerInfo(ret,"[w3ReceiveSearchQueryResultAsXml()]["+mode+"]");
		return ret;
	}
	...
	public int receiveSearchQueryResultAsJson(int mode) {
		...
		loggerInfo(ret,"[w3ReceiveSearchQueryResultAsJson()]["+mode+"]");
		return ret;
	}
	...
	public int recvDocument(int mode) {
		...
		loggerInfo(ret,"[w3ReceiveUidResult()]["+mode+"]");
		return ret;
	}
	...
	public int receiveUidResultAsXml(int mode) {
		...
		loggerInfo(ret,"[w3ReceiveUidResultAsXml()]["+mode+"]");
		return ret;
	}
	...
	public int receiveUidResultAsJson(int mode) {
		...
		loggerInfo(ret,"[w3ReceiveUidResultAsJson()]["+mode+"]");
		return ret;
	}
	...
	public int recvAnalyzedQuery(int mode) {
		...
		loggerInfo(ret,"[w3ReceiveMorphemeAnalysisResult()]["+mode+"]");
		return ret;
	}
	...
	public int receiveMorphemeAnalysisResultAsXml(int mode) {
		...
		loggerInfo(ret,"[w3ReceiveMorphemeAnalysisResultAsXml()]["+mode+"]");
		return ret;
	}
	...
	public int receiveMorphemeAnalysisResultAsJson(int mode) {
		...
		loggerInfo(ret,"[w3ReceiveMorphemeAnalysisResultAsJson()]["+mode+"]");
		return ret;
	}
	...
	public int addMergeCollectionInfo(String mergeCollection, String[] collections, int start, int count ){
		...
		for(int i=0; i < collectionLen; i++) {
			...
			loggerInfo(ret,"[w3AddMergeCollection()]["+mergeCollection+"]["+collections[i]+"]");
		}
		ret = search.w3SetMergePageInfo(mergeCollection, start, count);
		loggerInfo(ret,"[w3SetMergePageInfo()]["+mergeCollection+"]["+start+"]["+count+"]");
	...
	public int addMergeDocumentField(String mergeCollection, String[] mergeFields, String[] collections, String[][] fields) {
		...
		for(int i=0; i < collectionLen; i++) {
			for(int j=0; j<fieldsLen; j++) {
				...
				loggerInfo(ret,"[w3AddMergeDocumentField()]["+mergeCollection+"]["+mergeFields[j]+"]["+collections[i]+"]["+fields[i][j]+"]");
			}
		}
		return ret;
	}
	...
	*/
	public int addMergeMultiGroupByField(String mergeCollection, String[] mergeFields, String[] collections, String[][] fields) {
		...
		for(int i=0; i < collectionLen; i++) {
			for(int j=0; j<fieldsLen; j++) {
				...
				loggerInfo(ret,"[w3AddMergeMultiGroupByField()]["+mergeCollection+"]["+mergeFields[j]+"]["+collections[i]+"]["+fields[i][j]+"]");
			}
		}
		return ret;
	}
		...
		int collectionLen = collections.length;
	 	...
	 	for(int i=0; i < collectionLen; i++) {
	 		for(int j=0; j<fieldsLen; j++) {
	 			...
	 			loggerInfo(ret,"[w3AddMergeCategoryGroupByField()]["+mergeCollection+"]["+mergeFields[j]+"]["+collections[i]+"]["+fields[i][j]+"]");
	 		}
	 	}
	 	return ret;
	}
	...
	// log4에 남길지 여부 확인
	public void loggerInfo(int ret , String msg){
		if(this.isDebug){
			LOGGER.info(msg+"[ret:"+ret+"]");
		}
	}
